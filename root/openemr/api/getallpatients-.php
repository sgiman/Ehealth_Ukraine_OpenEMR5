...
if ($userId = validateToken($token)) {
    ...
    if ($acl_allow) {
        ...
        if ($numRows > 0) {
            ...
            $xml_string .= "<reason>Запис списку пацієнтів отримано</reason>";
            $counter = 0;
            ...
            while ($res = sqlFetchArray($result)) {
                ...
                if ($result1) {
					...
					//$file = getUrl($result1['url']); 	
					//chmod($file, 0777);				

                    $xml_string .= "<profileimage>" . $file . "</profileimage>\n";
            ...
            $xml_string .= "<reason>ERROR: На жаль, сталася помилка при обробці ваших даних. Будь ласка, надішліть інформацію ще раз.</reason>\n";
        }
    } else {
        $xml_string .= "<status>-2</status>\n";
        $xml_string .= "<reason>Ви не авторизовані для виконання цих дій</reason>\n";
    }
} else {
    $xml_string .= "<status>-2</status>\n";
    $xml_string .= "<reason>Неверний токен</reason>\n";
}
