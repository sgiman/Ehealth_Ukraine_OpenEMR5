...
if ($userId = validateToken($token)) {
    ...
    if ($acl_allow) {
        ...
        if ($patient) {
            ...
            $patient_data = getPatientNotes_SG($p_id);
            if ($patient_data) {
...
//  getPatientNotes_SG
/////////////////////////
function getPatientNotes_SG($pid = '', $limit = '', $offset = 0, $search = '')
{
  if($limit){
  ...
  $sql = "
    ...
    SELECT p.id, p.date, p.user, p.title,
           
      p.message_status, p.body
